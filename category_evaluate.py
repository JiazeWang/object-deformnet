...
parser.add_argument('--resume_model', type=str, default='rebuttal/r3.pth', help='resume from saved model')
parser.add_argument('--result_dir', type=str, default='rebuttal/t3_1', help='directory to save train results')
...
def train_net():
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        category_cd_loss = np.zeros((opt.n_cat,))
        ...
        for i, data in enumerate(val_dataloader, 1):
            ...
            if pred_sRT is not None:
                category_cd_loss[cat_id] += cd_loss
