...
from lib.network_t3_r5_eval import DeformNet
...
parser.add_argument('--model', type=str, default='results/T3_R5_camera_fast/model_44.pth', help='resume from saved model')
...
if opt.data == 'val':
    result_dir = 'results/'+str(opt.model).split('/')[1]+'_val2'
    file_path = 'CAMERA/val_list.txt'
    cam_fx, cam_fy, cam_cx, cam_cy = 577.5, 577.5, 319.5, 239.5
else:
    result_dir = 'results/'+str(opt.model).split('/')[1]+'_real_test2'
...
def detect():
    ...
    for path in tqdm(img_list):
        ...
        if len(valid_inst):
            ...
            #assign_mat, deltas, _, _, _, _, _ = estimator(f_points, f_rgb, f_choose, f_catId, f_prior, nocs, model)
            del(f_rgb)
            del(f_catId)
