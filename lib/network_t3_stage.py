...
class DeformNet(nn.Module):
    ...
    def forward(self, points, img, choose, cat_id, prior, nocs, model):
        ...
        loss =  loss1*1 + loss2*2 + loss3*4
        #loss = loss0*1 + loss1*2 + loss2*1 + loss3*0.5
        corr_loss = corr_loss1 + corr_loss2 + corr_loss3
        cd_loss =  cd_loss1 + cd_loss2 + cd_loss3
        entropy_loss = entropy_loss1 + entropy_loss2 + entropy_loss3
        deform_loss = deform_loss1 + deform_loss2 + deform_loss3
        """
