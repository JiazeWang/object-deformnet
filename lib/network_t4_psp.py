...
import numpy
from lib.pspnet_fpn import PSPNet
...
class DeformNet(nn.Module):
    def __init__(self, n_cat=6, nv_prior=1024):
        ...
        self.transformer64_0 = Transformer(emb_dims=64, N=1)
        self.transformer64_1 = Transformer(emb_dims=64, N=1)
        self.transformer64_2 = Transformer(emb_dims=64, N=1)
        self.transformer64_3 = Transformer(emb_dims=64, N=1)
        self.transformer128_0 = Transformer(emb_dims=128, N=1)
        self.transformer128_1 = Transformer(emb_dims=128, N=1)
        self.transformer128_2 = Transformer(emb_dims=128, N=1)
        self.transformer128_3 = Transformer(emb_dims=128, N=1)
    ...
    def forward(self, points, img, choose, cat_id, prior, nocs, model):
        ...
        di = out_img.size()[1]
        emb3 = out_img.view(bs, di, -1)
        choose = choose.unsqueeze(1).repeat(1, di, 1)
        emb3 = torch.gather(emb3, 2, choose).contiguous()

        di2 = p2.size()[1]
        emb2 = p2.view(bs, di2, -1)
        img_width = out_img.size()[2]
        emb2 = torch.gather(emb2, 2, choose).contiguous()

        di1 = p1.size()[1]
        emb1 = p1.view(bs, di1, -1)
        img_width = p2.size()[2]
        emb1 = torch.gather(emb1, 2, choose).contiguous()

        di0 = p0.size()[1]
        emb0 = p0.view(bs, di0, -1)
        img_width = p1.size()[2]
        emb0 = torch.gather(emb0, 2, choose).contiguous()

        emb0 = self.instance_color0(emb0)
        emb1 = self.instance_color1(emb1)
        emb2 = self.instance_color2(emb2)
        emb3 = self.instance_color3(emb3)
        ...
        # deformation field
        deform_feat1 = cat_global1     # bs x 2112 x n_pts
        ...
        # deformation field
        deform_feat2 = cat_global2      # bs x 2112 x n_pts
        ...
        loss = loss0*0.5 + loss1*1 + loss2*2 + loss3*4
