...
parser.add_argument('--result_dir', type=str, default='results/camera_t4_d4_half', help='directory to save train results')
...
def train_net():
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        val_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=1, sampler=train_sampler,
                                                       num_workers=opt.num_workers, pin_memory=True)
        estimator.eval()
